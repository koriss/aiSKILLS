...
def main():
    root=root_arg(); p=root/'execution/cron-ledger.jsonl'
    records=read_jsonl(p)
    for r in records:
        if r.get('mutated_active_rfo_run'): fail('cron mutated active RFO run')
        if r.get('contended_with_interactive_rfo'): fail('cron contended with interactive RFO')
