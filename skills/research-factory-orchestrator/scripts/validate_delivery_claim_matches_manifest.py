...
def main():
    ...
    if delivery.get('delivery_status') in ('not_queued','created','failed') and gate.get('passed') is True:
        return emit('fail','F303', message='final gate passed while delivery manifest not delivered')
    return emit('pass','F303')
