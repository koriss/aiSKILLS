...
2. **B2 — Deterministic replay smoke** — `scripts/_smoke_deterministic_replay.py` runs `validate()` twice on `good/minimal_valid` with identical B1 env; normalized `validation-transcript.json` must match (`DETERMINISTIC-DRIFT` on failure).
...
6. **C2 — Clean-install smoke** — `scripts/_smoke_clean_install.py` unzips the release zip to a temp tree and runs `validate_skill`, `check_schema_drift`, and `run_core_validators` on `good/minimal_valid` from that tree.
