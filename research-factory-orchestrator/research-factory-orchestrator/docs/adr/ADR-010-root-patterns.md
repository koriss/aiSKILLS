...
3. **Release path not exercising the real artifact path** — CI must include `artifact_execute_v19_3` + `validate_artifact_release` (see `scripts/validate_release.REQUIRED_GATES`), not scaffold-only substitutes.
