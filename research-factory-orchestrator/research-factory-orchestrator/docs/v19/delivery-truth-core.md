...
| `external_delivery_claim_allowed` | User-visible channel (host-owned messenger, webhook, …) proves delivery |
...
4. **ack_namespace:** matches provider (`cli:` local synthetic, `webhook:` …).
...
7. **no_local_path_leak:** scan **user_visible_artifacts** list (not only `chat/message-*.txt`): includes HTML/MD reports, `delivery-summary.json`, `verification-report.html`, compact chat payloads, error snippets copied into chat.
