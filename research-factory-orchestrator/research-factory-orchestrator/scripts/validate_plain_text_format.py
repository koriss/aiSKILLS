...
def main():
    ...
    if HTML_TAG.search(text):
        errors.append("plain-text chunk contains HTML-like markup")
    if MD_LINK.search(text):
        errors.append("plain-text chunk contains markdown link")
    if MARKDOWN_BOLD.search(text):
        errors.append("plain-text chunk contains markdown bold/underline")
    if MD_TABLE_ROW.search(text):
        errors.append("plain-text chunk contains markdown table row")

    if errors:
        print("\n".join(errors), file=sys.stderr)
        return 1
    print("OK: plain-text chunk is plain text")
    return 0
