...
This script is intentionally stdlib-only and safe to run from a host checkout.

``validator_id`` in JSON output is **verify_skill_run_claims** — the canonical
...
VALIDATOR_ID = "verify_skill_run_claims"
...
def main() -> int:
    ...
    gates = dm.get("gates") if isinstance(dm.get("gates"), dict) else {}
    ext_gate_from_gates = gates.get("external_delivery_gate") if isinstance(gates.get("external_delivery_gate"), dict) else {}
    out = {
        ...
        "artifact_signals": {
            ...
            "external_delivery_gate_status": ext_gate_from_gates.get("status"),
        },
    }
