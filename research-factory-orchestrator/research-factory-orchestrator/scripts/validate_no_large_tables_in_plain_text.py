...
def main():
    ...
    if rows:
        if len(rows) > 5:
            errors.append(f"plain-text table has too many rows: {len(rows)} > 5")
        max_cols = max(line.count("|") - 1 for line in rows)
        if max_cols > 3:
            errors.append(f"plain-text table has too many columns: {max_cols} > 3")
        if any(word in text.lower() for word in ["source registry", "coverage matrix", "evidence map", "fact-check table"]):
            errors.append("large analytical table detected in plain-text message")

    if errors:
        print("\n".join(errors), file=sys.stderr)
        return 1
    print("OK: no large plain-text tables")
    return 0
