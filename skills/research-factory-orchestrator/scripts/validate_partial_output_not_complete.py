...
def main():
    root=root_arg(); records=read_jsonl(root/'execution/model-call-ledger.jsonl')
    for r in records:
        if r.get('status') in ['partial','timeout','disconnect','overflow'] and r.get('finalize_seen') and r.get('used_as_complete'):
            fail('partial/timeout output used as complete')
        if r.get('status')=='complete' and r.get('finalize_seen') is False:
            fail('complete model call without finalize marker')
