...
def main():
    root=root_arg(); records=read_jsonl(root/'execution/late-results-ledger.jsonl')
    for r in records:
        if r.get('arrived_after') in ['retry_started','synthesis_started','report_finalized'] and r.get('silently_merged'):
            fail('late result silently merged')
