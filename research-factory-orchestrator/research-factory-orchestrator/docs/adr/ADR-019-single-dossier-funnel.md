...
2. **Relay bridge** (`scripts/run_rfo_with_web_search.py`) runs **multi-vector query fanout** (templates in `contracts/query-fanout-config.json`), merges/deduplicates URLs, and records **`relay_query_fanout`** / **`query_vectors`** statistics on `collection-result.json`.
